...
if pgrep -f "port-forward.*chaosmesh-microservice" &> /dev/null; then
  echo "App service is already running."
else
  echo "Starting app service on port 3000..."
  nohup kubectl port-forward --address 0.0.0.0 svc/chaosmesh-microservice 3000:80 > /tmp/chaosmesh-app.log 2>&1 &
